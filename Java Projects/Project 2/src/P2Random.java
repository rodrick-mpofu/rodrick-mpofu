...
public class P2Random {
    ...
    /**
     * Generate random orders for Project 2
     * @param seed the initial seed for the random number generator
     * @param numTraders total number traders, producing values [0, numTraders)
     * @param numStocks total number of stocks, producing values [0, numStocks)
     * @param numOrders total number of random orders to generate
     * @param arrivalRate the average number of orders per timestamp
     * @return a {@link Scanner} object ready to read the orders
     */
    public static Scanner PRInit(int seed, int numTraders, int numStocks, int numOrders, int arrivalRate) {
        ...
        return new Scanner(
                Stream.generate( () -> {
                    ...
                    int stockNum = (int) (Integer.toUnsignedLong(rd.nextInt()) % numStocks);
                    int traderNum = (int) (Integer.toUnsignedLong(rd.nextInt()) % numTraders);

                    String intent = (int) Integer.toUnsignedLong(rd.nextInt()) % 2 != 0 ? "BUY" : "SELL";
                    int quantity = (int) (Integer.toUnsignedLong(rd.nextInt()) % MAX_QUANTITY + 1);
                    int price = (int) (Integer.toUnsignedLong(rd.nextInt()) % MAX_FORCE_SENSITIVITY + 1);

                    return String.format("%d %s T%d S%d $%d #%d",
                ...
                }).limit(numOrders).collect(Collectors.joining("\n"))
        );
    }
